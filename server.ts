...
if (!adminCheck) {
  db.prepare('INSERT INTO admins (username, password) VALUES (?, ?)').run('SDN008Sangsel', 'Sangsel008SDN');
}

// Seed Settings if not exists
const schoolNameCheck = db.prepare("SELECT * FROM settings WHERE key = 'school_name'").get();
if (!schoolNameCheck) {
  db.prepare("INSERT INTO settings (key, value) VALUES (?, ?)").run('school_name', 'SDN 008 Sangatta Selatan');
}
...
if (!visiCheck) {
  db.prepare("INSERT INTO settings (key, value) VALUES (?, ?)").run('visi', 'Terwujudnya Generasi Unggul yang Berakhlak Mulia, Sehat, Kreatif dan Adaptif serta Berbudaya Lingkungan');
}
...
if (!contactAddressCheck) {
  db.prepare("INSERT INTO settings (key, value) VALUES (?, ?)").run('contact_address', 'Jl. Poros Sangatta-Bontang Km.28');
}
const contactPhoneCheck = db.prepare("SELECT * FROM settings WHERE key = 'contact_phone'").get();
if (!contactPhoneCheck) {
  db.prepare("INSERT INTO settings (key, value) VALUES (?, ?)").run('contact_phone', '085191572726');
}
const contactEmailCheck = db.prepare("SELECT * FROM settings WHERE key = 'contact_email'").get();
if (!contactEmailCheck) {
  db.prepare("INSERT INTO settings (key, value) VALUES (?, ?)").run('contact_email', 'sdn008sangsel@gmail.com');
}
const socialFacebookCheck = db.prepare("SELECT * FROM settings WHERE key = 'social_facebook'").get();
if (!socialFacebookCheck) {
  db.prepare("INSERT INTO settings (key, value) VALUES (?, ?)").run('social_facebook', 'SDN 008 SANGATTA SELATAN');
}
...
if (statsCheck.count === 0) {
  ...
  insertStat.run('Siswa', '120', '🎓');
  insertStat.run('Guru', '32', '👩‍🏫');
  insertStat.run('Kelas', '6', '🏫');
  insertStat.run('Prestasi', '36', '🏆');
}
...
if (teacherCheck.count === 0) {
  ...
  insertTeacher.run('Sunardi, M.Pd', 'Kepala Sekolah', '/uploads/placeholder-teacher.svg');
  insertTeacher.run('Fatimah, S.pd', 'Guru Agama', '/uploads/placeholder-teacher.svg');
  insertTeacher.run('Jumasir, S.pd', 'Guru Olahraga', '/uploads/placeholder-teacher.svg');
}
