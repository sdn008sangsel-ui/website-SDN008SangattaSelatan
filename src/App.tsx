    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
      ...
      >
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
          Mewujudkan Generasi <span className="text-blue-400">Berakhlak Mulia Sehat</span> & <span className="text-blue-400">Kreatif dan Adaptif</span>
        </h1>
        <p className="text-lg md:text-xl text-gray-200 mb-10 max-w-2xl mx-auto">
          Selamat Datang di Website Resmi {schoolName}. Tempat terbaik untuk menumbuhkan potensi anak sejak dini.
        </p>
      ...
      <div className="grid md:grid-cols-3 gap-12">
        <div>
          ...
          <p className="mb-6 text-gray-400">
            Terwujudnya Generasi Unggul yang Berakhlak Mulia, Sehat, Kreatif dan Adaptif serta Berbudaya Lingkungan.
          </p>
...
const AdminLogin = ({ onLogin, onClose }: { onLogin: (token: string) => void, onClose: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (username === 'SDN008Sangsel' && password === 'Sangsel008SDN') {
        onLogin('demo-token');
      } else {
...
export default function App() {
  ...
  const [schoolName, setSchoolName] = useState('SDN 008 Sangatta Selatan');
  ...
  const [visi, setVisi] = useState('Terwujudnya Generasi Unggul yang Berakhlak Mulia, Sehat, Kreatif dan Adaptif serta Berbudaya Lingkungan.');
  const [misi, setMisi] = useState([
    ...
    'Mendorong Kreativitas dan Sikap Adaptif.',
    'Menumbuhkan Kesadaran dan Kepedulian terhadap Lingkungan.'
  ]);
  const [registrationLink, setRegistrationLink] = useState('#');
  const [contactAddress, setContactAddress] = useState('Jl. Poros Sangatta-Bontang Km.28');
  const [contactPhone, setContactPhone] = useState('085191572726');
  const [contactEmail, setContactEmail] = useState('sdn008sangsel@gmail.com');
  const [socialFacebook, setSocialFacebook] = useState('SDN 008 SANGATTA SELATAN');
  ...
  const fetchData = async () => {
    try {
      ...
      if (!localStorage.getItem('teachers')) {
        const mockTeachers = [
          { id: 1, name: 'Sunardi,M.Pd', position: 'Kepala Sekolah', photoUrl: 'https://images.unsplash.com/photo-1568602471122-7832951cc4c5?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80' },
          { id: 2, name: 'Fatimah, S.Pd', position: 'Guru Agama', photoUrl: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80' },
          { id: 3, name: 'Jumasir', position: 'Guru PJOK', photoUrl: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80' }
        ];
      ...
      if (!localStorage.getItem('stats')) {
        const mockStats = [
          { id: 1, label: 'Siswa', value: '120', icon: '🎓' },
          { id: 2, label: 'Guru', value: '14', icon: '👩‍🏫' },
          { id: 3, label: 'Kelas', value: '6', icon: '🏫' },
          { id: 4, label: 'Prestasi', value: '51', icon: '🏆' }
        ];
        setStats(mockStats);
      ...
      if (!localStorage.getItem('settings')) {
         const mockSettings = {
            school_name: 'SDN 008 Sangatta Selatan',
            hero_image: 'https://images.unsplash.com/photo-1509062522246-3755977927d7?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80',
            visi: 'Terwujudnya Generasi Unggul yang Berakhlak Mulia, Sehat, Kreatif dan Adaptif serta Berbudaya Lingkungan.',
            misi: JSON.stringify([
           'Mengembangkan Potensi Akademik dan Non-Akademik Siswa.',
           'Membentuk Karakter Siswa yang Berakhlak Mulia.',
           'Menciptakan Lingkungan Sekolah yang Sehat dan Aman.',
           ...
           'Menumbuhkan Kesadaran dan Kepedulian terhadap Lingkungan.'
            ...
            registration_link: '#',
            contact_address: 'Jl. Poros Sangatta-Bontang Km.28',
            contact_phone: '085191572726',
            contact_email: 'sdn008sangsel@gmail.com',
            social_facebook: 'SDN 008 SANGATTA SELATAN',
            social_instagram: '#',
            social_twitter: '#',
            social_linkedin: '#'
         };
